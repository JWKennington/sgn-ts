...
class Multiplier(TSTransform):
    ...
    def new(self, pad):  # type: ignore
        ...
        minsegs = {
            str(n.name): [self.audioadapters[str(n.name)].slice[0]]
            for n in self.sink_pads
        }
        maxsegs = {
            str(n.name): [self.audioadapters[str(n.name)].slice[1]]
            for n in self.sink_pads
        }

        # Will only produce an output buffer with sum of the data in the overlap_segment
        overlap_segment = (
            max(minsegs.values())[0],
            min(maxsegs.values())[0],
        )  # finds the overlap of the offsets that we are working with
        ...
        if noffset <= 0:  # for when there is no overlap in the offsets
            return TSFrame(
                buffers=[
                    SeriesBuffer(
                        offset=offset, sample_rate=sample_rate, data=None, is_gap=True
                    )
                ],
                EOS=EOS,
                metadata=metadata,
            ...
            bothgaps = all(
                self.audioadapters[str(n.name)].is_gap for n in self.sink_pads
            )
            # Check if all gaps
            if bothgaps:
                return TSFrame(
            ...
            samples = (
                self.audioadapters[str(n.name)].copy_samples_by_offset_segment(
                    overlap_segment
                )
                for n in self.sink_pads
            )
            ...
            data = np.array(data)
            # data must be in tuple format for export and list format for calculateion
